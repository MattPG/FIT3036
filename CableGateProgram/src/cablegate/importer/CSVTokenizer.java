		...
		while( true ) {
			...
			if( TokenizerState.NORMAL.equals(state) ) {
				...
				if( c == NEWLINE ) {
					...
					/*
					 * The next character has been escaped. Append the escape character and the following character, then move
					 * the cursor so the next iteration of the loop will read the character following the escaped character
					 */
					currentColumn.append(ESCAPE_CHAR);
					currentColumn.append(line.charAt(++charIndex));
